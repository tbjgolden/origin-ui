...
function FilterQuickControls(props) {
  ...
  return <ButtonGroup size={SIZE.mini} kind={KIND.tertiary}>
    <Button type="button" onClick={props.onSelectAll}>{locale.datatable.categoricalFilterSelectAll}</Button>
    <Button type="button" onClick={props.onClearSelection}>{locale.datatable.categoricalFilterSelectClear}</Button>
  </ButtonGroup>;
}
...
function HighlightCheckboxLabel(props) {
  ...
  return splitByQuery(children, props.query).map((el, i) => {
    if (matchesQuery(el, props.query)) {
      return <StyledHighlightLabel {...restProps} key={i} $isFirst={!i} $isActive>{el}</StyledHighlightLabel>;
    }
    return <StyledHighlightLabel {...restProps} key={i} $isFirst={!i}>{el}</StyledHighlightLabel>;
  });
}
export function CategoricalFilter(props) {
  const [css, theme] = useStyletron();
  const locale = React.useContext(LocaleContext);
  const [selection, setSelection] = React.useState(props.filterParams ? props.filterParams.selection : /* @__PURE__ */ new Set());
  ...
  return <FilterShell exclude={exclude} onExcludeChange={() => {
    ...
    {showQuery && <Input size={INPUT_SIZE.compact} overrides={{ Before: InputBefore }} value={query} onChange={(event) => {
      return setQuery(event.target.value);
    }} clearable />}
    {!query && <div style={{
      marginTop: showQuery ? theme.sizing.scale600 : null
    }}><FilterQuickControls onSelectAll={() => {
    ...
    <div className={css({
      ...
      {filteredCategories.length === 0 && <LabelSmall>{locale.datatable.categoricalFilterEmpty}</LabelSmall>}
      {filteredCategories.length > 0 && filteredCategories.map((category, i) => {
        return <div className={checkboxStyles} key={i}><Checkbox checked={selection.has(category)} onChange={() => {
