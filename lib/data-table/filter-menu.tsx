...
function Options(props) {
  ...
  const buiRef = React.useRef(props.columns.map((col) => {
    return seed(col);
  }));
  ...
  return <div className={css({
    ...
    })}>{locale.datatable.optionsLabel}</p>
    {props.searchable && <div className={css({
      marginBottom: theme.sizing.scale500,
      marginRight: theme.sizing.scale600,
      marginLeft: theme.sizing.scale600
    })}><Input inputRef={inputRef} value={props.query} onChange={(event) => {
      return props.onQueryChange(event.target.value);
    }} placeholder={locale.datatable.optionsSearch} size={INPUT_SIZE.compact} clearable /></div>}
    {props.columns.length === 0 && <div className={css({
      ...theme.typography.font100,
      paddingRight: theme.sizing.scale600,
      paddingLeft: theme.sizing.scale600
    })}>{locale.datatable.optionsEmpty}</div>}
    <ul onKeyDown={props.onKeyDown} onFocus={handleFocus} onBlur={handleBlur} tabIndex="0" role="listbox" aria-activedescendant={`bui-${buiRef.current[props.highlightIndex]}`} className={css({
      listStyleType: "none",
      ...
      return <li id={`bui-${buiRef.current[index]}`} role="option" aria-selected={isHighlighted} onMouseEnter={() => {
        return props.onMouseEnter(index);
      }} onClick={() => {
        return props.onClick(column);
      }} key={column.title} className={css({
        ...theme.typography.font100,
        alignItems: "center",
...
function FilterMenu(props) {
  ...
  return <Popover focusLock returnFocus={true} placement={PLACEMENT.bottomLeft} content={() => {
    if (Filter && activeColumn) {
      return <Filter data={activeColumnData} close={handleClose} setFilter={(filterParams) => {
        return props.onSetFilter(activeColumn.title, filterParams);
      }} />;
    }
    return <Options columns={columns} highlightIndex={highlightIndex} onClick={handleOptionClick} onKeyDown={handleKeyDown} onMouseEnter={setHighlightIndex} onQueryChange={setQuery} query={query} searchable={filterableColumns.length >= 10} />;
  }} onClick={() => {
    if (isOpen) {
      handleClose();
    } else {
  ...
  }} onClickOutside={handleClose} onEsc={handleClose} isOpen={isOpen} ignoreBoundary><Button shape={SHAPE.pill} size={SIZE.compact} overrides={{
    BaseButton: {
      style: {
  ...
  }}>{locale.datatable.filterAdd}</Button></Popover>;
}
