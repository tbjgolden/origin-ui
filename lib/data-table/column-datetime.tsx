...
type OptionsT = SharedColumnOptionsT<Date> & {
  formatString?: string;
  locale?: any;
};
...
function DatetimeFilter(props) {
  ...
  const [rangeDates, setRangeDates] = React.useState<any>(
    initialState.rangeDates.length > 0
  ...
  return (
    <FilterShell
      ...
      onApply={() => {
        if (isRange) {
          const op: DatetimeOperationsT = rangeOperator[0].id;
        ...
        if (isCategorical) {
          const op: DatetimeOperationsT = categoricalOperator[0].id;
...
function DatetimeColumn(options: OptionsT): DatetimeColumnT {
  ...
  return Column({
    ...
    renderCell: function RenderDatetimeCell(props) {
      return (
        <DatetimeCell value={props.value} formatString={normalizedOptions.formatString} />
      );
    },
