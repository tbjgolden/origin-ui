...
const ListItem = React.forwardRef((props, ref) => {
  ...
  return <Root ref={ref} $shape={props.shape || SHAPE.DEFAULT} aria-label={props["aria-label"]} aria-selected={props["aria-selected"]} id={props.id} role={props.role} {...rootProps}>
    {Artwork && <ArtworkContainer $artworkSize={artworkSize} $sublist={Boolean(props.sublist)} {...artworkContainerProps}><Artwork size={typeof artworkSize === "number" ? artworkSize : artworkSizeToValue(artworkSize, Boolean(props.sublist))} /></ArtworkContainer>}
    <Content $mLeft={!Artwork} $sublist={!!props.sublist} {...contentProps}>
      {props.children}
      {EndEnhancer && EndEnhancer !== 0 && <EndEnhancerContainer {...endEnhancerContainerProps}><EndEnhancer /></EndEnhancerContainer>}
    </Content>
  </Root>;
});
