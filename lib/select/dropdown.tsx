...
export default class SelectDropdown extends React.Component {
  constructor() {
    ...
    this.getItemLabel = (option) => {
      ...
      let $selected;
      $selected = Array.isArray(value) ? !!value.find((selected) => {
      ...
      return <OptionContent aria-readonly={option.disabled} aria-selected={$selected} key={option[valueKey]} {...this.getSharedProps()} {...optionSharedProps} {...optionContentProps}>{getOptionLabel({ option, optionState: optionSharedProps })}</OptionContent>;
    };
    this.onMouseDown = (e) => {
  ...
  render() {
    ...
    return <DropdownContainer data-no-focus-lock ref={this.props.innerRef} {...this.getSharedProps()} {...dropdownContainerProps}><OverriddenStatefulMenu noResultsMsg={noResultsMsg} onActiveDescendantChange={(id) => {
      if (this.props.onActiveDescendantChange) {
        this.props.onActiveDescendantChange(id);
      }
    }} onItemSelect={onItemSelect} items={groupedOptions} size={size} initialState={{
      isFocused: true,
      highlightedIndex
    }} typeAhead={false} keyboardControlNode={this.props.keyboardControlNode} forceHighlight={true} overrides={mergeOverrides({
      List: {
    ...
    })} {...restStatefulMenuProps} /></DropdownContainer>;
  }
}
