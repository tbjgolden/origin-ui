...
function CountrySelectDropdown(props) {
  ...
  if (props.children.length === 0) {
    return <LocaleContext.Consumer>{(locale) => {
      return <EmptyState {...emptyStateProps}>{noResultsMsg || locale.menu.noResultsMsg}</EmptyState>;
    }}</LocaleContext.Consumer>;
  }
  const children = React.Children.toArray(props.children);
  const scrollIndex = Math.min(children.findIndex((opt) => {
    return opt.props.item.id === country.id;
  }) + 5, children.length - 1);
  return <Container ref={forwardedRef} $height={maxDropdownHeight} {...containerProps}><AutoSizer>{({ height, width }) => {
    return <List role="listbox" height={height} width={width} rowCount={children.length} rowHeight={42} scrollToIndex={scrollIndex} rowRenderer={({ index, key, style }) => {
      const { item, resetMenu, getItemLabel, ...rest } = children[index].props;
      const { id: iso, label, dialCode } = item;
      return <ListItem key={key} style={style} item={item} {...rest} {...listItemProps} data-iso={iso}>
        ...
        <NameColumn {...nameColumnProps}>{mapIsoToLabel ? mapIsoToLabel(iso) : label}</NameColumn>
