...
class StarRating extends React.Component {
  constructor() {
    ...
    this.renderRatingContents = () => {
      ...
      for (let x = 1; x <= numItems; x++) {
        ...
        refs.push(starRef);
        ratings.push(<Star key={x} role="radio" title="rating" ref={starRef} tabIndex={isFocusable ? "0" : "-1"} aria-setsize={numItems} aria-checked={x <= value} aria-posinset={x} aria-disabled={readOnly} $size={size} $index={x} $isActive={previewIndex !== void 0 ? x <= previewIndex : x <= value} $isPartialActive={previewIndex !== void 0 ? false : x <= value + 0.5} $isSelected={x === previewIndex} $isFocusVisible={this.state.isFocusVisible && isFocusable} $isReadOnly={readOnly} onClick={() => {
          if (readOnly) {
            return;
          }
          this.selectItem(x);
        ...
        }} {...starProps} onFocus={forkFocus(starProps, this.handleFocus)} onBlur={forkBlur(starProps, this.handleBlur)} />);
      }
      return ratings;
    };
  }
  render() {
    ...
    return <Root data-baseweb="star-rating" role="radiogroup" onBlur={(e) => {
