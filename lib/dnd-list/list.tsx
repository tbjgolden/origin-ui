...
class StatelessList extends React.Component {
  ...
  render() {
    ...
    return <Root $isRemovable={isRemovable} data-baseweb="dnd-list" {...rootProps} onFocus={forkFocus(rootProps, this.handleFocus)} onBlur={forkBlur(rootProps, this.handleBlur)}><MovableList removableByMove={isRemovableByMove} values={items} onChange={onChange} renderList={({ children, props, isDragged }) => {
      return <List $isRemovable={isRemovable} $isDragged={isDragged} ref={props.ref} {...listProps}>{children}</List>;
      ...
      return <ItemLayer dragged={isDragged} key={props.key}><Item {...sharedProps} ref={props.ref} tabIndex={props.tabIndex} aria-roledescription={props["aria-roledescription"]} onKeyDown={props.onKeyDown} onWheel={props.onWheel} {...itemProps} style={{ ...props.style, display: "flex" }}>
        <DragHandle {...sharedProps} {...dragHandleProps}><Grab size={24} /></DragHandle>
        <Label {...sharedProps} {...labelProps}>{value}</Label>
        {removable && <CloseHandle {...sharedProps} onClick={(evt) => {
          evt.preventDefault();
          onChange && onChange({
            oldIndex: typeof index !== "undefined" ? index : 0,
            newIndex: -1
          });
        }} {...closeHandleProps}><Delete size={24} color="#CCC" /></CloseHandle>}
      </Item></ItemLayer>;
    }} /></Root>;
  }
}
