  ...
  render() {
    ...
    return <ThemeContext.Consumer>{(theme) => {
      return <LocaleContext.Consumer>{(locale) => {
        return <Root data-baseweb="pagination" {...rootProps}>
          <Button aria-label={this.constructAriaWayfinderLabel(locale, "previous page. current page")} disabled={currentPage <= 1} onClick={this.onPrevClick} startEnhancer={() => {
            return theme.direction === "rtl" ? <ChevronRight title="" size={24} /> : <ChevronLeft title="" size={24} />;
          }} kind={KIND.tertiary} overrides={{
            BaseButton: overrides.PrevButton
          }} size={size}>{labels && labels.prevButton ? labels.prevButton : locale.pagination.prev}</Button>
          <DropdownContainer $isFocusVisible={this.state.isFocusVisible} {...dropdownContainerProps} onFocus={forkFocus(dropdownContainerProps, this.handleFocus)} onBlur={forkBlur(dropdownContainerProps, this.handleBlur)}><Select aria-label={this.constructAriaWayfinderLabel(locale, "page")} options={options} labelKey="label" valueKey="label" onChange={this.onMenuItemSelect} searchable={false} clearable={false} value={[{ label: currentPage }]} maxDropdownHeight="200px" size={size} overrides={{
            ControlContainer: {
          ...
          }} {...selectProps} /></DropdownContainer>
          <MaxLabel {...maxLabelProps} aria-hidden={true}>{`${labels && labels.preposition ? labels.preposition : locale.pagination.preposition || ""} ${numPages}`}</MaxLabel>
          <Button aria-label={this.constructAriaWayfinderLabel(locale, "next page. current page")} disabled={currentPage >= numPages} onClick={this.onNextClick} endEnhancer={() => {
            return theme.direction === "rtl" ? <ChevronLeft title="" size={24} /> : <ChevronRight title="" size={24} />;
          }} kind={KIND.tertiary} overrides={{
            BaseButton: overrides.NextButton
          }} size={size}>{labels && labels.nextButton ? labels.nextButton : locale.pagination.next}</Button>
        </Root>;
