  ...
  render() {
    ...
    function renderProgressBar() {
      ...
      for (let i = 0; i < steps; i++) {
        children.push(<Bar key={i} {...sharedProps} {...barProps}><BarProgress $index={i} {...sharedProps} {...barProgressProps} /></Bar>);
      }
      return children;
    }
    return <Root ref={forwardedRef} data-baseweb="progress-bar" role="progressbar" aria-label={ariaLabel || getProgressLabel(value, maximumValue, minValue)} aria-valuenow={infinite ? null : value} aria-valuemin={infinite ? null : minValue} aria-valuemax={infinite ? null : maximumValue} aria-invalid={errorMessage ? true : null} aria-errormessage={errorMessage} {...restProps} {...sharedProps} {...rootProps}>
      <BarContainer {...sharedProps} {...barContainerProps}>{infinite ? <React.Fragment>
        <InfiniteBar $isLeft={true} $size={sharedProps.$size} {...infiniteBarProps} />
        <InfiniteBar $size={sharedProps.$size} {...infiniteBarProps} />
      </React.Fragment> : renderProgressBar()}</BarContainer>
      {showLabel && <Label {...sharedProps} {...labelProps}>{getProgressLabel(value, maximumValue, minValue)}</Label>}
    </Root>;
  }
}
