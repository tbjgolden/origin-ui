...
const _BaseInput = class extends React.Component {
  ...
  renderClear() {
    ...
    return <ClearIconContainer $alignTop={this.props.type === CUSTOM_INPUT_TYPE.textarea} {...sharedProps} {...clearIconContainerProps}><ClearIcon size={iconSize} tabIndex={0} title={ariaLabel} aria-label={ariaLabel} onClick={this.onClearIconClick} onKeyDown={(event) => {
      if (event.key && (event.key === "Enter" || event.key === " ")) {
        event.preventDefault();
        this.onClearIconClick();
      }
    }} role="button" $isFocusVisible={this.state.isFocusVisibleForClear} {...sharedProps} {...clearIconProps} onFocus={forkFocus(clearIconProps, this.handleFocusForClear)} onBlur={forkBlur(clearIconProps, this.handleBlurForClear)} /></ClearIconContainer>;
  }
  render() {
    const {
      value,
    ...
    const autoComplete = this.state.initialType === "password" && this.props.autoComplete === _BaseInput.defaultProps.autoComplete ? "new-password" : this.props.autoComplete;
    const sharedProps = getSharedProps(this.props, this.state);
    ...
    return <InputContainer data-baseweb={this.props["data-baseweb"] || "base-input"} {...sharedProps} {...inputContainerProps}>
      <Before {...sharedProps} {...beforeProps} />
      <Input ref={this.inputRef} aria-activedescendant={this.props["aria-activedescendant"]} aria-autocomplete={this.props["aria-autocomplete"]} aria-controls={this.props["aria-controls"]} aria-errormessage={this.props["aria-errormessage"]} aria-haspopup={this.props["aria-haspopup"]} aria-label={this.props["aria-label"]} aria-labelledby={this.props["aria-labelledby"]} aria-describedby={this.props["aria-describedby"]} aria-invalid={this.props.error} aria-required={this.props.required} autoComplete={autoComplete} disabled={this.props.disabled} id={this.props.id} inputMode={this.props.inputMode} maxLength={this.props.maxLength} name={this.props.name} onBlur={this.onBlur} onChange={this.props.onChange} onFocus={this.onFocus} onKeyDown={this.props.onKeyDown} onKeyPress={this.props.onKeyPress} onKeyUp={this.props.onKeyUp} pattern={this.props.pattern} placeholder={this.props.placeholder} type={this.getInputType()} required={this.props.required} role={this.props.role} value={this.props.value} min={this.props.min} max={this.props.max} step={this.props.step} rows={this.props.type === CUSTOM_INPUT_TYPE.textarea ? this.props.rows : null} {...sharedProps} {...inputProps}>{type === CUSTOM_INPUT_TYPE.textarea ? value : null}</Input>
      {this.renderClear()}
