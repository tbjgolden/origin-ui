...
function OptionList(props, ref) {
  ...
  const getItem = (item2) => {
    return item2.href && renderHrefAsAnchor ? <ListItemAnchor $item={item2} href={item2.href} {...listItemAnchorProps}>{getItemLabel(item2)}</ListItemAnchor> : <>{getItemLabel(item2)}</>;
  };
  return <LocaleContext.Consumer>{(locale) => {
    return <MaybeChildMenu getChildMenu={getChildMenu} isOpen={!!$isHighlighted} item={item} resetParentMenu={resetMenu} renderAll={renderAll} onClick={onClick} overrides={overrides}><ListItem ref={ref} aria-label={getChildMenu && getChildMenu(item) ? locale.menu.parentMenuItemAriaLabel : null} item={item} onClick={onClick} onMouseEnter={onMouseEnter} $size={size} $isHighlighted={$isHighlighted} {...restProps} {...listItemProps}>{getItem({ isHighlighted: $isHighlighted, ...item })}</ListItem></MaybeChildMenu>;
