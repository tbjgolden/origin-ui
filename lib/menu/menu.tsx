...
export default function Menu(props) {
  ...
  const [elements] = optgroups.reduce(([els, itemIndex], optgroup) => {
    if (optgroup !== "__ungrouped") {
      els.push(<OptgroupHeader key={optgroup} {...optgroupHeaderProps}>{optgroup}</OptgroupHeader>);
    }
    const groupItems = groupedItems[optgroup].map((item, index) => {
      ...
      return <Option renderAll={renderAll} key={itemIndex} item={item} overrides={props.overrides} resetMenu={resetMenu} role="option" $disabled={disabled} $isFocused={isFocused} $isHighlighted={isHighlighted} aria-disabled={disabled} aria-selected={isHighlighted && isFocused} {...restProps} {...optionProps} />;
    });
  ...
  return <LocaleContext.Consumer>{(locale) => {
    return <List aria-activedescendant={props.activedescendantId || null} role="listbox" aria-label={ariaLabel} ref={rootRef} onMouseEnter={focusMenu} onMouseLeave={handleMouseLeave} onMouseOver={focusMenu} onFocus={forkFocus({ onFocus: focusMenu }, handleFocus)} onBlur={forkBlur({ onBlur: unfocusMenu }, handleBlur)} onKeyDown={(event) => {
      if (props.isFocused) {
        handleKeyDown(event);
      }
    }} tabIndex={0} data-baseweb="menu" $isFocusVisible={focusVisible} {...listProps}>{isEmpty ? <EmptyState aria-live="polite" aria-atomic {...emptyStateProps}>{props.noResultsMsg || locale.menu.noResultsMsg}</EmptyState> : elements}</List>;
