...
function Slider({
  ...
  return <Root data-baseweb="slider" {...sharedProps} {...rootProps} onFocus={forkFocus(rootProps, handleFocus)} onBlur={forkBlur(rootProps, handleBlur)}>
    <Range step={step} min={min} max={max} values={value} disabled={disabled} onChange={(value2) => {
      return onChange({ value: value2 });
    }} onFinalChange={(value2) => {
      return onFinalChange({ value: value2 });
    }} rtl={theme.direction === "rtl"} renderTrack={({ props, children, isDragged }) => {
      return <Track onMouseDown={props.onMouseDown} onTouchStart={props.onTouchStart} $isDragged={isDragged} {...sharedProps} {...trackProps}><InnerTrack $isDragged={isDragged} ref={props.ref} {...sharedProps} {...innerTrackProps}>{children}</InnerTrack></Track>;
    }} renderThumb={({ props, index, isDragged }) => {
      const displayLabel = persistentThumb ? persistentThumb : (!!index && isHovered1 || !index && isHovered0 || isDragged) && !disabled;
      return <Thumb {...props} onMouseEnter={() => {
        if (index === 0) {
          setIsHovered0(true);
        } else {
          setIsHovered1(true);
      ...
      }} $thumbIndex={index} $isDragged={isDragged} style={{
        ...props.style
      }} {...sharedProps} {...thumbProps} $isFocusVisible={isFocusVisible && focusedThumbIndex === index}>
        {displayLabel && <ThumbValue $thumbIndex={index} $isDragged={isDragged} {...sharedProps} {...thumbValueProps}>{valueToLabel(value[index])}</ThumbValue>}
        {displayLabel && <InnerThumb $thumbIndex={index} $isDragged={isDragged} {...sharedProps} {...innerThumbProps} />}
      </Thumb>;
    }} {...marks ? {
      renderMark: ({ props, index }) => {
        return <Mark $markIndex={index} {...props} {...sharedProps} {...markProps} />;
      }
    } : {}} />
    <TickBar {...sharedProps} {...tickBarProps}>
      <Tick {...sharedProps} {...tickProps}>{valueToLabel(min)}</Tick>
      <Tick {...sharedProps} {...tickProps}>{valueToLabel(max)}</Tick>
    </TickBar>
  </Root>;
}
