...
export default class PinCode extends React.Component {
  ...
  getMaskStyle(i) {
    return this.props.values[i] !== "" && typeof this.props.mask === "string" ? this.props.mask : this.props.values[i];
  }
  render() {
    ...
    return <Root data-baseweb="pin-code" {...rootProps}>{this.props.values.map((v, i) => {
      return <Input aria-label={this.props["aria-label"]} aria-labelledby={this.props["aria-labelledby"]} aria-describedby={this.props["aria-describedby"]} autoComplete={this.props.autoComplete} disabled={this.props.disabled} error={this.props.error} id={this.props.id ? this.props.id + "-" + i : null} inputMode="numeric" inputRef={this._inputRefs.ref(i)} key={i} name={this.props.name} onBlur={() => {
        ...
        if (eventValue.length > 2) {
          if (eventValue.length === this.props.values.length && /^\d+$/.test(eventValue)) {
      ...
      }} pattern="\d*" placeholder={this.state.hasFocus ? "" : this.props.placeholder} positive={this.props.positive} required={this.props.required} size={this.props.size} value={this.getMaskStyle(i)} {...inputProps} />;
    })}</Root>;
  }
}
