...
const Tag = React.forwardRef((props, ref) => {
  ...
  return <Root ref={ref} data-baseweb="tag" aria-label={isButton && closeable ? `${typeof children === "string" ? `${children}, ` : ""}close by backspace` : null} aria-disabled={disabled ? true : null} role={isButton ? "button" : null} tabIndex={isButton ? 0 : null} {...rootHandlers} {...sharedProps} {...rootProps} onFocus={forkFocus(rootProps, handleFocus)} onBlur={forkBlur(rootProps, handleBlur)}>
    {StartEnhancer && StartEnhancer !== 0 && <StartEnhancerContainer {...startEnhancerContainerProps}><StartEnhancer /></StartEnhancerContainer>}
    <Text title={titleText} {...textProps}>{children}</Text>
    {closeable ? <Action aria-hidden={true} role="presentation" {...actionHandlers} {...sharedProps} {...actionProps}><ActionIcon size={actionSize} {...actionIconProps} /></Action> : null}
  </Root>;
});
Tag.displayName = "Tag";
export default Tag;
