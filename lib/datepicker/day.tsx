...
export default class Day extends React.Component {
  constructor(props) {
    ...
    this.getMonthProp = () => {
      return this.props.month === void 0 || this.props.month === null ? this.dateHelpers.getMonth(this.getDateProp()) : this.props.month;
    };
  ...
  componentDidMount() {
    if (this.dayElm && this.props.focusedCalendar && (this.props.highlighted || !this.props.highlightedDate && this.isSelected())) {
      this.dayElm.focus();
    }
  }
  ...
  isSelected() {
    ...
    return Array.isArray(value) ? this.dateHelpers.isSameDay(date, value[0]) || this.dateHelpers.isSameDay(date, value[1]) : this.dateHelpers.isSameDay(date, value);
  }
  isPseudoSelected() {
    const date = this.getDateProp();
    const { value } = this.props;
  ...
  isPseudoHighlighted() {
    ...
    if (Array.isArray(value)) {
      ...
      if (highlightedDate && start && !end) {
        return this.dateHelpers.isAfter(highlightedDate, start) ? this.dateHelpers.isDayInRange(this.clampToDayStart(date), this.clampToDayStart(start), this.clampToDayStart(highlightedDate)) : this.dateHelpers.isDayInRange(this.clampToDayStart(date), this.clampToDayStart(highlightedDate), this.clampToDayStart(start));
      }
      if (highlightedDate && !start && end) {
        return this.dateHelpers.isAfter(highlightedDate, end) ? this.dateHelpers.isDayInRange(this.clampToDayStart(date), this.clampToDayStart(end), this.clampToDayStart(highlightedDate)) : this.dateHelpers.isDayInRange(this.clampToDayStart(date), this.clampToDayStart(highlightedDate), this.clampToDayStart(end));
      }
    }
  }
  getSharedProps() {
  ...
  render() {
    ...
    return !peekNextMonth && sharedProps.$outsideMonth ? <Day2 role="gridcell" {...sharedProps} {...dayProps} onFocus={this.onFocus} onBlur={this.onBlur} /> : <LocaleContext.Consumer>{(locale) => {
      return <Day2 aria-label={this.getAriaLabel(sharedProps, locale)} ref={(dayElm) => {
        this.dayElm = dayElm;
      }} role="gridcell" aria-roledescription="button" tabIndex={this.props.highlighted || !this.props.highlightedDate && this.isSelected() ? 0 : -1} {...sharedProps} {...dayProps} onFocus={this.onFocus} onBlur={this.onBlur} onClick={this.onClick} onKeyDown={this.onKeyDown} onMouseOver={this.onMouseOver} onMouseLeave={this.onMouseLeave}>
        <div>{this.dateHelpers.getDate(date)}</div>
        {dateLabel ? <DayLabel {...sharedProps} {...dayLabelProps}>{dateLabel}</DayLabel> : null}
      </Day2>;
