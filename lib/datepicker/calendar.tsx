...
export default class Calendar extends React.Component {
  constructor(props) {
    ...
    this.renderCalendarHeader = (date = this.state.date, order) => {
      return <CalendarHeader {...this.props} key={`month-header-${order}`} date={date} order={order} onMonthChange={this.changeMonth} onYearChange={this.changeYear} />;
    };
    ...
    this.handleArrowKey = (key) => {
      ...
      switch (key) {
        case "ArrowLeft":
          highlightedDate2 = this.dateHelpers.subDays(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
        case "ArrowRight":
          highlightedDate2 = this.dateHelpers.addDays(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
        case "ArrowUp":
          highlightedDate2 = this.dateHelpers.subWeeks(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
        case "ArrowDown":
          highlightedDate2 = this.dateHelpers.addWeeks(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
        case "Home":
          highlightedDate2 = this.dateHelpers.getStartOfWeek(highlightedDate2 ? highlightedDate2 : currentDate);
          break;
        case "End":
          highlightedDate2 = this.dateHelpers.getEndOfWeek(highlightedDate2 ? highlightedDate2 : currentDate);
          break;
        case "PageUp":
          highlightedDate2 = this.dateHelpers.subMonths(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
        case "PageDown":
          highlightedDate2 = this.dateHelpers.addMonths(highlightedDate2 ? highlightedDate2 : currentDate, 1);
          break;
      }
    ...
    this.renderMonths = (translations) => {
      ...
      for (let i = 0; i < (this.props.monthsShown || 1); ++i) {
        ...
        monthSubComponents.push(<CalendarContainer key={monthKey} ref={(calendar) => {
          this.calendar = calendar;
        }} role="grid" aria-roledescription={translations.ariaRoleDescCalMonth} aria-multiselectable={this.props.range || null} onKeyDown={this.onKeyDown} {...calendarContainerProps} $density={this.props.density}><Month adapter={this.props.adapter} date={monthDate} dateLabel={this.props.dateLabel} density={this.props.density} excludeDates={this.props.excludeDates} filterDate={this.props.filterDate} highlightedDate={this.state.highlightedDate} includeDates={this.props.includeDates} focusedCalendar={this.state.focused} range={this.props.range} locale={this.props.locale} maxDate={this.props.maxDate} minDate={this.props.minDate} month={this.dateHelpers.getMonth(this.state.date)} onDayBlur={this.blurCalendar} onDayFocus={this.onDayFocus} onDayClick={this.props.onDayClick} onDayMouseOver={this.onDayMouseOver} onDayMouseLeave={this.onDayMouseLeave} onChange={this.handleDateChange} overrides={overrides} value={this.props.value} peekNextMonth={this.props.peekNextMonth} fixedHeight={this.props.fixedHeight} hasLockedBehavior={!!this.props.hasLockedBehavior} selectedInput={this.props.selectedInput} /></CalendarContainer>);
        monthList.push(<div key={`month-component-${i}`}>{monthSubComponents}</div>);
      }
      return <MonthContainer $orientation={orientation} {...monthContainerProps}>{monthList}</MonthContainer>;
    };
    this.renderTimeSelect = (value2, onChange, label) => {
      ...
      return <TimeSelectContainer {...timeSelectContainerProps}><TimeSelectFormControl label={label} {...timeSelectFormControlProps}><TimeSelect value={value2 ? this.dateHelpers.date(value2) : value2} onChange={onChange} nullable {...timeSelectProps} /></TimeSelectFormControl></TimeSelectContainer>;
    };
    this.renderQuickSelect = () => {
      ...
      const [QuickSelect, { overrides: quickSelectOverrides, ...restQuickSelectProps }] = getOverrides(overrides.QuickSelect, Select);
      ...
      return <LocaleContext.Consumer>{(locale) => {
        return <QuickSelectContainer {...quickSelectContainerProps}><QuickSelectFormControl label={locale.datepicker.quickSelectLabel} {...quickSelectFormControlProps}><QuickSelect aria-label={locale.datepicker.quickSelectAriaLabel} labelKey="id" onChange={(params) => {
          if (!params.option) {
        ...
        ]} placeholder={locale.datepicker.quickSelectPlaceholder} value={this.state.quickSelectId && [{ id: this.state.quickSelectId }]} overrides={mergeOverrides({
          Dropdown: {
            style: {
              textAlign: "start"
            }
          }
        }, quickSelectOverrides)} {...restQuickSelectProps} /></QuickSelectFormControl></QuickSelectContainer>;
    ...
    this.state = {
      highlightedDate: this.getSingleDate(value) || (highlightedDate && this.dateHelpers.isSameMonth(dateInView, highlightedDate) ? highlightedDate : this.dateHelpers.date()),
      focused: false,
      date: dateInView,
      quickSelectId: null,
      rootElement: null,
  ...
  setHighlightedDate(date) {
    ...
    let nextState;
    nextState = selected && this.dateHelpers.isSameMonth(selected, date) && this.dateHelpers.isSameYear(selected, date) ? { highlightedDate: selected } : {
  ...
  render() {
    ...
    return <LocaleContext.Consumer>{(locale) => {
      return <Root $density={this.props.density} data-baseweb="calendar" role="application" aria-roledescription="datepicker" ref={(root) => {
        if (root && root instanceof HTMLElement && !this.state.rootElement) {
          this.setState({
            rootElement: root
          });
        }
      }} aria-label={locale.datepicker.ariaLabelCalendar} onKeyDown={this.props.trapTabbing ? this.handleTabbing : null} {...rootProps}>
        {this.renderMonths({
          ariaRoleDescCalMonth: locale.datepicker.ariaRoleDescriptionCalendarMonth
        })}
        {this.props.timeSelectStart && this.renderTimeSelect(startDate, (time) => {
          return this.handleTimeChange(time, 0);
        }, locale.datepicker.timeSelectStartLabel)}
        {this.props.timeSelectEnd && this.props.range && this.renderTimeSelect(endDate, (time) => {
