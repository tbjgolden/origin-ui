...
export default class TableBuilder extends React.Component {
  ...
  render() {
    ...
    const columns = React.Children.toArray(children).filter(Boolean).map((child) => {
      return child.props;
    });
    function renderHeader(col, colIndex, isFocusVisible2) {
      ...
      if (!col.sortable) {
        ...
        return <ColTableHeadCell key={colIndex} $col={col} $colIndex={colIndex} $divider={divider} $isNumeric={col.numeric} $size={size} {...tableHeadCellProps} {...colTableHeadCellProps}>{col.header}</ColTableHeadCell>;
      }
      ...
      switch (col.id === sortColumn && sortOrder) {
        case "ASC":
          sortIcon = <SortAscIcon size="16px" aria-hidden={true} role="presentation" {...sortAscIconProps} />;
          sortLabel = "ascending sorting";
          break;
        case "DESC":
          sortIcon = <SortDescIcon size="16px" aria-hidden={true} role="presentation" {...sortDescIconProps} />;
          sortLabel = "descending sorting";
          break;
        default:
          sortIcon = <SortNoneIcon size="16px" aria-hidden={true} role="presentation" {...sortNoneIconProps} />;
          break;
      }
      return <ColTableHeadCellSortable key={colIndex} $col={col} $colIndex={colIndex} $divider={divider} $isNumeric={col.numeric} role="button" tabIndex="0" aria-label={`${col.tableHeadAriaLabel || col.header}, ${sortLabel}`} $isFocusVisible={isFocusVisible2} onClick={() => {
      ...
      }} {...tableHeadCellSortableProps} {...colTableHeadCellSortableProps}>
        {col.header}
    ...
    function renderCell(col, colIndex, row, rowIndex, lastRowindex) {
      ...
      return <ColTableBodyCell key={colIndex} $col={col} $colIndex={colIndex} $divider={divider} $row={row} $rowIndex={rowIndex} $isNumeric={col.numeric} $isLastRow={rowIndex === lastRowindex} $isSortable={col.sortable} $size={size} {...tableBodyCellProps} {...colTableBodyCellProps}>{col.children(row, rowIndex)}</ColTableBodyCell>;
    }
    const isEmpty = !isLoading && data.length === 0;
    const isRendered = !isLoading && !isEmpty;
    return <Root data-baseweb="table-builder-semantic" $divider={divider} {...rootProps} {...rest}><Table $width={horizontalScrollWidth} {...tableProps} onBlur={forkBlur(tableProps, this.handleBlur)} onFocus={forkFocus(tableProps, this.handleFocus)}>
      <TableHead {...tableHeadProps}><TableHeadRow {...tableHeadRowProps}>{columns.map((col, colIndex) => {
        return renderHeader(col, colIndex, this.state.isFocusVisible);
      })}</TableHeadRow></TableHead>
      <TableBody {...tableBodyProps}>
        {isLoading && <tr><td colSpan={columns.length}><TableLoadingMessage {...tableLoadingMessageProps}>{typeof loadingMessage === "function" ? loadingMessage() : loadingMessage}</TableLoadingMessage></td></tr>}
        {isEmpty && emptyMessage && <tr><td colSpan={columns.length}><TableEmptyMessage {...tableEmptyMessageProps}>{typeof emptyMessage === "function" ? emptyMessage() : emptyMessage}</TableEmptyMessage></td></tr>}
        {isRendered && data.map((row, rowIndex) => {
          return <TableBodyRow key={rowIndex} $divider={divider} $row={row} $rowIndex={rowIndex} {...tableBodyRowProps}>{columns.map((col, colIndex) => {
